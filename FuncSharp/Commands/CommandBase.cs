        ...
        if (RunAs != null)
        {
            if (!RunAs.Contains(' '))
            {
                execute += $"run execute if entity {RunAs} as {RunAs} run {GenerateInternal()}\n" +
                           ...
                           $"execute unless entity {RunAs} run execute as @e[type=marker,tag=funcsharp_run_at_helper, limit=1] at {RunAt} run {GenerateInternal()}\n" +
                           $"execute unless entity RunAs run kill @e[type=marker,tag=funcsharp_run_at_helper, limit=1]";
            }
            else
            {
